...
### ⏳ Вынос в отдельный `<RarityCard/>` — отложено, зафиксировано на будущее (2026-09-21)

Сейчас весь паттерн (словарь `RARITY_STYLES`, разметка контейнера/spotlight/CTA) живёт целиком внутри
`CategoryEntryCard.tsx` — это не переиспользуемый примитив, а конкретная карточка входа в категорию.

**Когда вернуться к вопросу**: как только rarity-паттерн понадобится вне `CategoryEntryCard` —
например, reward-тир в `GamificationCompact`/профиле, предмет в инвентаре лутбокса, история призов.
До этого — не выносить (правило "техническое ≠ визуальное": рефакторинг без второго потребителя —
риск без пользы).

**Что делать, когда понадобится**: вынести `RARITY_STYLES` + разметку border/shadow/glow/spotlight
в `src/components/RarityCard.tsx` как примитив, принимающий `rarity` + `children`/content-пропы.
`CategoryEntryCard` становится тонкой обёрткой над ним. Источник паттерна — этот раздел
(`🧩 Компонентный паттерн: Rarity Card`) и текущая реализация в `CategoryEntryCard.tsx` как эталон.
...
- **`npx tsc -p tsconfig.app.json --noEmit`** + `npm run build` перед каждым коммитом.
  ⚠️ НЕ `npx tsc --noEmit` без `-p` — корневой `tsconfig.json` в этом репо solution-style
  (`"files": []`), он ничего не компилирует и всегда молча возвращает код 0, даже при реальных
  ошибках типов. Только прогон с `-p tsconfig.app.json` реально проверяет `src/`.
