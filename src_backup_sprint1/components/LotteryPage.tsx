  ...
  return (
    <motion.div style={{ textAlign: 'center' }} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <span style={{
        ...
        {display.toLocaleString()}<span style={{ fontSize: compact ? 11 : 14, marginLeft: 3, opacity: 0.75, fontFamily: "'Space Grotesk', sans-serif" }}>TON</span>
        <span style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', color: 'var(--ton)', animation: 'glitchShift 0.5s ease-in-out infinite alternate', clipPath: 'polygon(0 0, 100% 0, 100% 35%, 0 55%)', pointerEvents: 'none' }}>{display.toLocaleString()}<span style={{ fontSize: compact ? 11 : 14, marginLeft: 3, opacity: 0.75 }}>TON</span></span>
        <span style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', color: 'var(--coral)', animation: 'glitchShift 0.7s ease-in-out infinite alternate-reverse', clipPath: 'polygon(0 55%, 100% 35%, 100% 100%, 0 100%)', pointerEvents: 'none' }}>{display.toLocaleString()}<span style={{ fontSize: compact ? 11 : 14, marginLeft: 3, opacity: 0.75 }}>TON</span></span>
      </span>
    </motion.div>
  );
}
...
/* ── Segmented Countdown ── */
function SegmentedCountdown({ target }: { target: string }) {
  ...
  const ds: React.CSSProperties = {
    ...
    background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(255,107,53,0.25)',
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    fontSize: 16, fontWeight: 600, color: '#FF8C42',
    fontFamily: "'Geist Mono', monospace", textShadow: '0 0 10px rgba(255,107,53,0.5)',
  ...
  return (
    <div style={{ textAlign: 'center', padding: '4px 0 8px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8 }}>
        <div style={{ textAlign: 'center' }}><div><span style={ds}>{p.h[0]}<span style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'rgba(255,255,255,0.06)' }} /></span><span style={ds}>{p.h[1]}<span style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'rgba(255,255,255,0.06)' }} /></span></div><span style={{ fontSize: 7, fontWeight: 600, textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', marginTop: 4, letterSpacing: '0.12em', fontFamily: "'Geist Mono', monospace" }}>Hours</span></div>
        <span style={{ fontSize: 16, fontWeight: 600, color: 'rgba(255,255,255,0.15)', fontFamily: "'Geist Mono', monospace", paddingBottom: 12 }}>:</span>
        <div style={{ textAlign: 'center' }}><div><span style={ds}>{p.m[0]}<span style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'rgba(255,255,255,0.06)' }} /></span><span style={ds}>{p.m[1]}<span style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'rgba(255,255,255,0.06)' }} /></span></div><span style={{ fontSize: 7, fontWeight: 600, textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', marginTop: 4, letterSpacing: '0.12em', fontFamily: "'Geist Mono', monospace" }}>Min</span></div>
        <span style={{ fontSize: 16, fontWeight: 600, color: 'rgba(255,255,255,0.15)', fontFamily: "'Geist Mono', monospace", paddingBottom: 12 }}>:</span>
        <div style={{ textAlign: 'center' }}><div><span style={ds}>{p.s[0]}<span style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'rgba(255,255,255,0.06)' }} /></span><span style={ds}>{p.s[1]}<span style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'rgba(255,255,255,0.06)' }} /></span></div><span style={{ fontSize: 7, fontWeight: 600, textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', marginTop: 4, letterSpacing: '0.12em', fontFamily: "'Geist Mono', monospace" }}>Sec</span></div>
      </div>
    </div>
  );
}
...
export function LotteryPage() {
  ...
  const lotteryId = params.slug || 'daily-rush';
  const lottery = LOTTERIES.find((l: Lottery) => l.id === lotteryId || l.id === 'daily-rush') || LOTTERIES[0];
  const accent = lottery.accentColor;
  const MAX = lottery.numbersCount;
  ...
  const randomPick = useCallback(() => {
    const pool = Array.from({ length: lottery.numbersMax }, (_, i) => i + 1);
    for (let i = pool.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [pool[i], pool[j]] = [pool[j], pool[i]]; }
    setSelected(pool.slice(0, MAX));
  }, [MAX, lottery.numbersMax]);
    ...
    <div style={{ position: 'relative', minHeight: '100vh', background: '#06080D', overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 390, margin: '0 auto', padding: '0 16px 110px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 20, background: 'rgba(6,8,13,0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', padding: '12px 0 4px', margin: '0 -16px', paddingLeft: 16, paddingRight: 16, paddingTop: 'calc(12px + env(safe-area-inset-top, 0px))' }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', padding: '4px 10px', borderRadius: 8, background: 'rgba(244,63,94,0.18)', color: '#F43F5E', border: '1px solid rgba(244,63,94,0.3)', fontFamily: "'Geist Mono', monospace" }}>LIVE</span>
        </div>
        ...
        <div style={{ textAlign: 'center', padding: '8px 0 0' }}>
          ...
          <GlitchJackpot value={lottery.jackpot} />
        </div>

        <SegmentedCountdown target={lottery.nextDraw} />
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
            {Array.from({ length: lottery.numbersMax }, (_, i) => i + 1).map(n => {
              const isSelected = selected.includes(n);
              const rank = hotOrder.indexOf(n) + 1;
              const t = rank / lottery.numbersMax;
        ...
        <div style={{ ...glassCard, gap: 0, padding: 14, borderRadius: 16 }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', lineHeight: 1.7, fontFamily: "'Space Grotesk', sans-serif" }}>
            Select <span style={{ color: accent, fontWeight: 600 }}>{MAX} numbers from {lottery.numbersMax}</span>. Match <span style={{ color: accent, fontWeight: 600 }}>2/{MAX}</span> → 70% of pool | <span style={{ color: accent, fontWeight: 600 }}>3/{MAX}</span> → 30% | <span style={{ color: 'var(--gold)', fontWeight: 600 }}>{MAX}/{MAX}</span> → Jackpot.
          </div>
        </div>

        {/* ── Prize cards — glass morphism ── */}
        <div style={{ display: 'flex', gap: 10 }}>
          {[{ m: `2/${MAX}`, s: '70%', l: 'Prize Pool' }, { m: `3/${MAX}`, s: '30%', l: 'Prize Pool' }, { m: `${MAX}/${MAX}`, s: 'Jackpot', l: `${lottery.jackpot} TON`, jp: true }].map((pz, i) => (
            <div key={i} style={{
              ...glassCard,
              ...
              <div style={{ fontSize: 18, fontWeight: 700, color: pz.jp ? 'var(--gold)' : '#fff', fontFamily: "'Geist Mono', monospace" }}>{pz.m}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: pz.jp ? 'var(--gold)' : accent, marginTop: 4, fontFamily: "'Geist Mono', monospace" }}>{pz.s}</div>
        ...
        <div style={{ textAlign: 'center', fontSize: 9, color: 'rgba(255,255,255,0.15)', fontFamily: "'Geist Mono', monospace" }}>
          {lottery.drawLabel} · {lottery.description}
        </div>
      </div>
      ...
      <div style={{ position: 'sticky', bottom: 64, zIndex: 15, background: 'linear-gradient(180deg, transparent 0%, rgba(6,8,13,0.94) 45%)', padding: '12px 16px 10px' }}>
        ...
        >
          <span style={{ opacity: added ? 0 : 1, transition: 'opacity 0.25s' }}>Add to Cart · {lottery.ticketPrice} {lottery.currency}</span>
