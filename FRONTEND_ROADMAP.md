...
- [ ] **Второй blue `#0EA5E9` (техдолг T2, в работе):** не отдельный дубль, а целая cyan-рампа (`#38BDF8 → #0EA5E9 → #0284C7 → #015AA0`) в `.htp-btn`, `.ticket-badge`, `.glitch-b`, иконках. Сведение к `--primary` = видимое изменение cyan→blue, требует скрин-проверки
