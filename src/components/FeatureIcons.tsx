// Neon glyph icons for the Features banner — matches the "Dark Vault" canon
// used across the app (num-cell.selected, hero jackpot): accent color reads as
// LIGHT, not paint. Pure crisp strokes, no duotone fills, neon glow driven by
// `currentColor` + CSS drop-shadow. Kept separate from AnimatedIcons.tsx so the
// richer multi-color icons used elsewhere (CardDecor) stay untouched.
...
// The neon glow itself is applied on the parent `.feature-card-icon` in CSS.
function IconShell({
  size = 34,
  color = 'currentColor',
  anim,
...
// Faint accent core-line under the main stroke to add "filament" depth to the
// neon without reintroducing a flat duotone fill.
const CORE = 0.28;
...
export function RocketIcon({ size, color }: IconProps) {
  return (
    <IconShell size={size} color={color} anim="feature-icon-float">
      ...
      <path d="M12 14.5v3.4" strokeOpacity={CORE} />
    </IconShell>
  );
}
...
export function CoinIcon({ size, color }: IconProps) {
  return (
    <IconShell size={size} color={color} anim="feature-icon-pulse">
      ...
      <path d="M8.2 9.2h7.6L12 16.4 8.2 9.2Z" />
      <path d="M12 9.2v7.2" />
      <circle cx="12" cy="12" r="5" strokeOpacity={CORE} />
    </IconShell>
  );
}
...
export function ContractIcon({ size, color }: IconProps) {
  return (
    <IconShell size={size} color={color} anim="feature-icon-pulse">
      ...
      <path d="M8.5 8h7M8.5 11.5h7M8.5 15h3" />
...
export function DiamondIcon({ size, color }: IconProps) {
  return (
    <IconShell size={size} color={color} anim="feature-icon-shimmer">
      ...
      <path d="M3 9h18" />
      <path d="M9 4l-2.5 5L12 20M15 4l2.5 5L12 20" strokeOpacity={CORE} />
    </IconShell>
  );
}
